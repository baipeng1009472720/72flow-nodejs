...
export class NodeExecutorFactory {
  static async execute(node: NodeDef, context: FlowContext): Promise<NodeResult> {
    ...
    switch (type) {
      case 'START':    return StartExecutor.execute(node, context);
      case 'END':      return EndExecutor.execute(node, context);
      case 'SCRIPT':   return ScriptExecutor.execute(node, context);
      case 'DECISION':
      case 'CONDITION': return DecisionExecutor.execute(node, context);
      case 'PARALLEL': return ParallelExecutor.execute(node, context);
      case 'LOOP':     return await LoopExecutor.execute(node, context);
      case 'API':      return await ApiExecutor.execute(node, context);
      case 'LLM':      return await LlmExecutor.execute(node, context);
      default:
  ...
  static execute(node: NodeDef, context: FlowContext): NodeResult {
    ...
    const scriptCode: string | undefined =
      typeof scriptCfg === 'string'
        ? scriptCfg
        : (scriptCfg?.scriptCode ?? scriptCfg?.code);
    ...
    try {
      ...
      const proxy = new Proxy(vars, {
        ...
        has(_target, key) { return true; }, // with(){} 需要 has 返回 true
      });
      ...
      // 如果展开 { ...vars, ...writes }，FlowContext.complete() 会把全量变量再写一遍，
      // 可能导致数字变量被之前某次迭代的字符串值覆盖（如 "1" + 1 = "11"）
...
class DecisionExecutor {
  static execute(node: NodeDef, context: FlowContext): NodeResult {
    ...
    if (type === 'DECISION') {
      ...
      let selectedBranch: string | null = null;
      for (const edge of outgoing) {
        if (selectedBranch) { context.skipNode(edge.to); continue; }
        let matched = false;
